  ...
  <div class="public-browse">
    ...
    <header class="header">
      ...
      <div class="header-right">
        ...
        <span class="file-count">{{ totalCount }} {{ $t('common.files') }}</span>
      </div>
    </header>
    ...
    <div v-else-if="error" class="error-container">
      <p>{{ error }}</p>
      <button v-if="canRetry" @click="loadFiles" class="retry-btn">{{ $t('common.retry') }}</button>
      <div class="error-credit">
        <p>{{ $t('publicBrowse.creditBy') }}</p>
            ...
            <div class="image-wrapper" :class="{ loaded: file.loaded }">
              ...
              <div class="overlay">
                <div class="overlay-actions">
                  ...
                  <button class="action-btn" @click.stop="downloadFile(file.name)" :title="$t('common.download')">
    ...
    <div v-if="previewVisible" class="preview-modal" @click.self="closePreview">
      ...
      <!-- 桌面端旋转按钮 -->
      <button class="rotate-btn" @click.stop="rotateImage" v-if="!isMobile" :title="$t('publicBrowse.rotate')">
      ...
      try {
        ...
        if (err.response?.status === 403) {
          ...
          if (msg.includes('disabled')) {
            this.error = this.$t('publicBrowse.featureDisabled');
          } else if (msg.includes('not allowed') || msg.includes('No public')) {
            this.error = this.$t('publicBrowse.dirNotAllowed');
          } else {
