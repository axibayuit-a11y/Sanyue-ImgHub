    ...
    >
        ...
        <template #footer>
            <span class="dialog-footer">
                <el-button @click="handleClose">{{ $t('tags.close') }}</el-button>
            </span>
        </template>
    </el-dialog>
</template>
    ...
    methods: {
        async loadFileTags() {
            try {
                ...
                ElMessage.error(this.$t('message.loadTagsFailed'));
            }
        },
        ...
        async handleAddTag() {
            ...
            if (this.currentTags.includes(tag)) {
                ElMessage.warning(this.$t('message.tagExists'));
                this.inputTag = '';
                this.showSuggestions = false;
                return;
            }
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    this.inputTag = '';
                    this.showSuggestions = false;
                    ElMessage.success(this.$t('message.tagAdded'));
                    ...
                    throw new Error(error.message || this.$t('message.operationFailed'));
                }
            } catch (error) {
                console.error('Error adding tag:', error);
                ElMessage.error(error.message || this.$t('message.operationFailed'));
            }
        },
        ...
        async handleRemoveTag(tag) {
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    this.currentTags = data.tags || [];
                    ElMessage.success(this.$t('message.tagRemoved'));
                    this.$emit('tagsUpdated', this.currentTags);
